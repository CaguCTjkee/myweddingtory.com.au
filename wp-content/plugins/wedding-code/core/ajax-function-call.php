...
function wedding_wp_new_user_notification( $user_id, $plaintext_pass = '' ) {
		...
		$headers = 'From: noreply  <noreply@myweddingtory.com.au>' . "\r\n".
...
if( !function_exists('wedding_ajax_forgot_pass') ){
	...
	function wedding_ajax_forgot_pass()
	{
        ...
        //$headers = 'From: No Reply <noreply@myweddingtory.com.au>' . "\r\n";
       	$headers = 'From: noreply  <noreply@myweddingtory.com.au>' . "\r\n".
				'Reply-To: noreply@myweddingtory.com.au'."\r\n" .
				'X-Mailer: PHP/' . phpversion();
        ...
        */
        ...
        $subject = __('Password Reset Request Body','weddingvendor');
        ...
        $msg .= __('To reset your password, visit the following address:','weddingvendor') . '<br>';
        $msg .= wedding_tg_validate_url($post_id,$type) . "action=reset_pwd&key=$key&login=" . rawurlencode($user_login);


        $subject = __('Password Reset Request','weddingvendor');
        $headers = array('Content-Type: text/html; charset=UTF-8');

        wp_mail( trim($user_email), $subject, $msg,$headers);
        ...
        /*if (@wp_mail($user_email, __('Password Reset Request','weddingvendor'), $message,  $headers) ) {
                echo '<div>'.__('We have just sent you an email with Password reset instructions.','weddingvendor').'</div>';
                exit();
        }
        else {
...
function wedding_ajax_credit_card_payment()
{
	...
	if($ack == "SUCCESS")
	{
		...
		{
			$expired_date= date('Y-m-d', strtotime("+1 year", strtotime($user_payment_expired_date)));
		...
		{
			$expired_date= date('Y-m-d', strtotime("+1 year", strtotime($cur_date)));
			update_user_meta( $userID, 'user_payment_expired_date', $expired_date ) ;
